...
function startMusic() {
  ...
  showToastMessage('Music Started', 'Fantasy ambience is now playing. Browser audio requires a click, so this button starts the audio context.');
}

function lockPointer() {
  const blocked = !started || !codexPanel.classList.contains('hidden') || !trialModal.classList.contains('hidden') || !detailPanel.classList.contains('hidden');
  if (!blocked) canvas.requestPointerLock?.();
}
...
function onMouseWheel(event) {
  ...
  player.fov = clamp(player.fov + Math.sign(event.deltaY) * 4, 38, 88);
  worldClickHint = player.fov <= 46 ? 'Zoomed in.' : player.fov >= 82 ? 'Zoomed out.' : 'Mouse wheel zoom adjusted.';
}
...
function onWorldClick() {
  ...
  if (point) setNavigationTarget(point.x, point.z);
  else interactWithFocused();
}

function onKeyDown(event) {
  const key = event.key.toLowerCase();
  if (movementKeys.has(key)) event.preventDefault();
  if (!detailPanel.classList.contains('hidden') && key === 'escape') { event.preventDefault(); closeDetailPanel(); return; }
  keys.add(key);
  if (key === 'escape') document.exitPointerLock?.();
  if (key === 'c') openCodex();
  if (key === 'm') openCommonsMap();
...
function loop(ts) {
  ...
  scene.update({ player, activeRealm, discoveredArtifacts: save.data.discoveredArtifacts, repositories: repositoryPayload, cameraBeat: beat, navTarget });
  focusedInteraction = started ? scene.getFocusedInteraction(player, 860) : null;
...
function update(dt) {
  ...
  let nearestRealm = null, nearestRealmDistance = Infinity;
  ...
  for (const realm of realms) {
    ...
    if (d < 285) {
      ...
      audio.setRealm(realm.id);
    }
  }
  vaultNear = distanceTo(0, 850) < 190;
  if (vaultNear) { world.corvusText = corvusLines.vault?.[0] || 'Contact Tower is nearby.'; audio.setRealm('idle'); }
  ...
  if (activeRealm !== priorRealm) {
    if (activeRealm === 'extreme-sports') unlock('extremeSports');
    if (activeRealm === 'martial') unlock('martialHall');
  ...
  if (Date.now() - lastSaveAt > 1500) { save.savePlayer({ x: player.x, z: player.z, yaw: player.yaw, pitch: player.pitch, fov: player.fov }); lastSaveAt = Date.now(); }
...
function setNavigationTarget(x, z) {
  ...
  worldClickHint = 'Traveling. WASD takes manual control.';
}

function travelToRealm(id) {
  const realm = realms.find((item) => item.id === id);
  if (!realm) return;
  setNavigationTarget(realm.x, realm.z - 175);
  save.visitRealm(realm.id);
  detailPanel.classList.add('hidden');
  detailContent.innerHTML = '';
  showToastMessage(`Traveling to ${realm.name}`, realm.plain || realm.quest || 'Following the village road.');
...
function interactWithFocused() {
  if (!focusedInteraction) {
    ...
    else showToastMessage('Exploration Mode', 'Center the reticle on a named building, artifact, tablet, river, or open ground. Press M for the map.');
    return;
  }
  ...
  else if (type === 'river') openDetail('Mill River', 'Mill River', '<p>The river is the main visual route through the village. It should guide travel rather than float over the land.</p>');
...
function renderInteractionPrompt() {
  if (!started) return;
  if (!focusedInteraction) {
    interactionPrompt.textContent = vaultNear ? 'Click or press E to enter Contact Tower' : (worldClickHint || 'Press M for the map. Click open ground to move. Press E on named targets.');
  ...
  else if (type === 'terrain') interactionPrompt.textContent = navTarget ? 'Traveling. WASD cancels.' : 'Click open ground to walk there';
...
function inspectArtifact(id) {
  ...
  if (artifact.action === 'github') return openProjectWorkshop();
  showArtifact(id);
}

function inspectTablet(index) {
  if (!tablets[index]) return;
  if (save.unlockTablet(index)) { audio.accent(); if (save.data.tablets.length >= 6) unlock('tabletKeeper'); }
...
function inspectPortal(id) {
  ...
  save.visitRealm(id);
  activeRealm = id;
  if (id === 'workshop') return openProjectWorkshop();
  openRealmDetail(realm);
  updateHud(realm, 0);
}

function openRealmDetail(realm) {
  const artifactCards = (realm.artifacts || []).map((artifactId) => renderArtifactCard(artifactId)).join('');
  openDetail(realm.short, realm.name, `${renderRealmHero(realm)}<p><strong>${escapeHTML(realm.title)}</strong></p><p>${escapeHTML(realm.body)}</p><p>${escapeHTML(realm.plain || realm.quest)}</p><div class="realm-actions"><button data-travel-realm="${escapeAttribute(realm.id)}">Travel here</button></div><h3>Inside this destination</h3><div class="district-grid">${artifactCards}</div>`);
  bindMapActions();
}
...
function showArtifact(id) {
  ...
  const project = projectShowcase[id];
  const rich = project ? renderProjectShowcase(project) : '';
  openDetail(`${realm?.name || 'Commons'} / ${artifact.type}`, artifact.title, `<p>${escapeHTML(artifact.body)}</p>${rich}${metaPills([artifact.type, realm?.name || 'Village'])}`);
}

function openAboutJeff() {
  openDetail('About Jeff Barnes', 'Builder, Architect, Leader', `<div class="profile-reader"><img src="${jeffProfileImage}" alt="Jeff Barnes portrait" onerror="this.src='${fallbackPortrait()}'"><div>${resumeData.about.map((p) => `<p>${escapeHTML(p)}</p>`).join('')}${metaPills(['Healthcare analytics', 'Enterprise architecture', 'AI governance', 'Data engineering', 'Leadership', 'Martial arts', 'Extreme sports'])}<p>${linkButton(socialProfile.linkedin, 'Open LinkedIn')}</p></div></div>`);
}

function openCommonsMap() {
  openDetail('Village Map', 'Jeff Barnes Commons', `${renderInteractiveMap()}<p class="map-note">This is now a working travel map. Choose any named destination to auto-walk there, or open its detail panel.</p>`);
  bindMapActions();
}

function renderInteractiveMap() {
  const bounds = { minX: -1220, maxX: 1220, minZ: -780, maxZ: 980 };
  const xPct = (x) => ((x - bounds.minX) / (bounds.maxX - bounds.minX)) * 100;
  const yPct = (z) => (1 - ((z - bounds.minZ) / (bounds.maxZ - bounds.minZ))) * 100;
  const lines = realms.map((realm) => `<line x1="50%" y1="45%" x2="${xPct(realm.x)}%" y2="${yPct(realm.z)}%" />`).join('');
  const nodes = realms.map((realm) => `<button class="map-poi" style="left:${xPct(realm.x)}%;top:${yPct(realm.z)}%;--poi:${escapeAttribute(realm.color)}" data-travel-realm="${escapeAttribute(realm.id)}"><span>${escapeHTML(realm.name)}</span></button>`).join('');
  const cards = realms.map((realm) => `<article class="map-destination-card"><h3>${escapeHTML(realm.name)}</h3><p>${escapeHTML(realm.plain || realm.title)}</p><div><button data-travel-realm="${escapeAttribute(realm.id)}">Travel</button><button data-open-realm="${escapeAttribute(realm.id)}">Open details</button></div></article>`).join('');
  return `<section class="interactive-map-shell"><div class="parchment-map"><svg viewBox="0 0 1200 720" preserveAspectRatio="none"><defs><filter id="roughInk"><feTurbulence type="fractalNoise" baseFrequency=".018" numOctaves="3"/><feColorMatrix type="saturate" values="0"/><feComponentTransfer><feFuncA type="table" tableValues="0 .13"/></feComponentTransfer><feBlend in="SourceGraphic"/></filter></defs><rect width="1200" height="720" rx="36" fill="#d9bd7a"/><path d="M60 520 C250 430 350 455 485 365 C650 255 770 310 910 230 C1025 165 1095 150 1160 120" fill="none" stroke="#326e7f" stroke-width="34" opacity=".62"/><path d="M115 360 C300 250 450 310 600 265 C790 205 885 250 1080 170" fill="none" stroke="#6f4b27" stroke-width="22" stroke-linecap="round" opacity=".54"/><g class="map-lines">${lines}</g><circle cx="50%" cy="45%" r="56" fill="#8d6530" stroke="#2a1709" stroke-width="6"/><text x="50%" y="46%" text-anchor="middle" font-family="Georgia" font-size="28" font-weight="700" fill="#29170b">Commons</text><rect x="24" y="24" width="1152" height="672" fill="none" stroke="#2c1708" stroke-width="8"/><rect x="52" y="52" width="1096" height="616" fill="none" stroke="#fff4ce" stroke-opacity=".28" stroke-width="3"/><g filter="url(#roughInk)"><path d="M45 660 C300 690 540 610 790 650 C1000 680 1115 615 1160 650" fill="none" stroke="#2a1709" stroke-opacity=".18" stroke-width="5"/></g></svg>${nodes}</div><div class="map-destination-grid">${cards}</div></section>`;
}

function bindMapActions() {
  detailContent.querySelectorAll('[data-travel-realm]').forEach((button) => button.addEventListener('click', () => travelToRealm(button.dataset.travelRealm)));
  detailContent.querySelectorAll('[data-open-realm]').forEach((button) => button.addEventListener('click', () => {
    const realm = realms.find((item) => item.id === button.dataset.openRealm);
    if (realm) openRealmDetail(realm);
  }));
  bindHashActionLinks();
}
...
function openProjectWorkshop() {
  const repos = repositoryPayload.length ? repositoryPayload : githubSeeds;
  const cards = repos.map((repo) => `<article class="repo-card"><p class="eyebrow">${escapeHTML(repo.status || 'repo')} / ${escapeHTML(repo.language || 'code')}</p><h3>${escapeHTML(repo.repo)}</h3><p>${escapeHTML(repo.title)}</p><div class="repo-signal"><span style="width:${clamp(Number(repo.signal) || 60, 10, 100)}%"></span></div><p>${linkButton(repo.url, 'Open repository')}</p></article>`).join('');
  openDetail('Projects', 'Project Workshop', `<div class="project-workshop-hero"><div><p class="eyebrow">Living code and experiments</p><h3>GitHub projects, prototypes, and active builds</h3><p>This is the builder’s section: ThreadlineAI, Project Odin, OpenAegis, OpenPulse, healthcare concepts, and code experiments.</p><p>${linkButton('https://github.com/grumpystrongman', 'Open GitHub Profile')}</p></div></div><div class="repo-grid">${cards}</div>`);
}

function showGalleryList() {
  const cards = galleryPayload.map((item) => renderGalleryCard(item)).join('');
  openDetail('Chronicle House', 'Recent Articles and Images', `<img class="detail-hero" src="${fantasyArt.chronicleHouse}" alt="Chronicle House artwork"><p>Imported posts open as full scrollable articles with images and original source links.</p><div class="detail-gallery-grid">${cards}</div>`);
}
...
function renderArtifactCard(artifactId) {
  const artifact = artifacts[artifactId];
  if (!artifact) return '';
  return `<article class="artifact-card"><p class="eyebrow">${escapeHTML(artifact.type || 'Artifact')}</p><h3>${escapeHTML(artifact.title)}</h3><p>${escapeHTML(artifact.body)}</p></article>`;
}

function renderRealmHero(realm) {
  const art = ({ about: fantasyArt.commonsMap, resume: fantasyArt.resumeHall, chronicle: fantasyArt.chronicleHouse, foundry: fantasyArt.workshop, ai: fantasyArt.workshop, healthcare: fantasyArt.healthcareHall, leadership: fantasyArt.leadershipLibrary, workshop: fantasyArt.workshop })[realm.id];
  if (art) return `<img class="detail-hero" src="${escapeAttribute(art)}" alt="${escapeAttribute(realm.name)} artwork">`;
  return `<div class="realm-hero-card" style="--realm:${escapeAttribute(realm.color)}"><span>${escapeHTML(realm.short)}</span><h3>${escapeHTML(realm.name)}</h3><p>${escapeHTML(realm.title)}</p></div>`;
}

function renderProjectShowcase(project) {
  return `<section class="project-showcase"><div><p class="eyebrow">${escapeHTML(project.status)}</p><h3>${escapeHTML(project.title)}</h3><p><strong>${escapeHTML(project.subtitle)}</strong></p><ul>${project.bullets.map((b) => `<li>${escapeHTML(b)}</li>`).join('')}</ul><p>${linkButton(project.url, 'Open working example / source')}</p></div></section>`;
}
...
function renderTrial(snapshot) {
  if (!snapshot) return;
  const actions = document.querySelector('.trial-actions');
  actions.innerHTML = Object.entries(snapshot.moves).map(([id, move]) => `<button data-trial="${id}">${move.label}</button>`).join('');
  actions.querySelectorAll('[data-trial]').forEach((button) => button.addEventListener('click', () => trial.choose(button.dataset.trial)));
...
function openCodex() {
  ...
  pauseOverlay.classList.add('hidden');
  save.noteCodexOpen();
  const realmRows = realms.map((realm) => `<article><h3>${escapeHTML(realm.name)}</h3><p><strong>${escapeHTML(realm.title)}</strong></p><p>${escapeHTML(realm.body)}</p></article>`).join('');
  const repoRows = repositoryPayload.map((repo) => `<li><strong>${escapeHTML(repo.repo)}</strong> — ${escapeHTML(repo.title)}<br><span>${escapeHTML(repo.realm)} / ${escapeHTML(repo.status)} / signal ${escapeHTML(String(repo.signal))}</span></li>`).join('');
  codexContent.innerHTML = `<div class="profile-reader"><img src="${jeffProfileImage}" alt="Jeff Barnes portrait" onerror="this.src='${fallbackPortrait()}'"><div><h3>${escapeHTML(socialProfile.name)}</h3><p><strong>${escapeHTML(resumeData.headline)}</strong></p><p>${escapeHTML(resumeData.about[0])}</p></div></div><h3>Village Locations</h3>${realmRows}<h3>Living Workshop</h3><ul>${repoRows}</ul><h3>Production Asset Pipeline</h3><ul>${assetManifest.productionNeeds.map((need) => `<li>${escapeHTML(need)}</li>`).join('')}</ul>`;
...
function startCameraBeat(id) {
  ...
  if (beat) { directorPanel.classList.remove('hidden'); directorText.textContent = `${beat.title}: ${beat.text}`; }
}

function renderDirector(beatState) {
  if (!beatState?.active) { if (directorPanel && !director.isPlaying) directorPanel.classList.add('hidden'); return; }
...
function updateHud(nearestRealm = null, distance = null) {
  ...
  if (activeRealm) {
    ...
    objectiveText.textContent = nearestRealm ? `Nearest named location: ${nearestRealm.name}. Distance ${Math.round(distance)}. Press M for the travel map.` : 'Press M for the map. Click open ground to move. WASD gives direct control.';
...
function renderMinimap() {
  ...
  const sx = (x) => w / 2 + x / 16;
  const sz = (z) => h / 2 + (260 - z) / 16;
  ctx.strokeStyle = 'rgba(240,194,111,.25)';
  realms.forEach((realm) => { ctx.beginPath(); ctx.moveTo(sx(0), sz(260)); ctx.lineTo(sx(realm.x), sz(realm.z)); ctx.stroke(); });
  realms.forEach((realm) => { ctx.fillStyle = realm.color; ctx.beginPath(); ctx.arc(sx(realm.x), sz(realm.z), activeRealm === realm.id ? 6 : 4, 0, Math.PI * 2); ctx.fill(); });
  ctx.fillStyle = '#f2c76d';
  ctx.fillRect(sx(0) - 4, sz(850) - 4, 8, 8);
  if (navTarget) { ctx.strokeStyle = '#fff0c8'; ctx.beginPath(); ctx.arc(sx(navTarget.x), sz(navTarget.z), 6, 0, Math.PI * 2); ctx.stroke(); }
  ctx.fillStyle = '#fff';
  ctx.beginPath();
...
function metaPills(values) { return `<div class="artifact-meta">${values.filter(Boolean).map((value) => `<span>${escapeHTML(value)}</span>`).join('')}</div>`; }
